...
export default function ImageGenerator() {
  ...
  const handleBatchDownloadZip = async () => {
    if (!editorRef.current || productImages.length <= 0) return
    ...
    try {
      ...
      // Save current image index to restore later
      ...
      for (let i = 0; i < productImages.length; i++) {
        // Generate the image
        const dataUrl = await generateImage(i)
        ...
        if (dataUrl) {
          ...
          zip.file(`instagram-${previewMode}-${i + 1}.png`, blob)
        }
      }
      ...
      // Save the zip file
      FileSaver.saveAs(content, `instagram-images-${new Date().toISOString().slice(0, 10)}.zip`)

      // Restore original image index
              ...
              <div>
                ...
                <div className="space-y-2">
                  ...
                  <Button
                    ...
                    variant="outline"
                    className="w-full"
                    disabled={productImages.length <= 0 || isDownloading}
                  >
                    <Download className="mr-2 h-4 w-4" />
                    {isDownloading ? "Creating ZIP..." : "Download All Images (ZIP)"}
                  </Button>
                </div>
              </div>
            </TabsContent>
